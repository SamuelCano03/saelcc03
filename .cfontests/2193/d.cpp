...
void solve(int caso){
  ...
  vi a(n), b(n);
  read(a,b);
  fore(i,n-1)b[i]+=b[i-1];
  si st; for(int e: a)st.insert(e);
  sort(all(a));
  int ans = 0;
  for(int e: st){
    int cnt = lower_bound(all(a),e)-a.begin();
    cnt = n - cnt;
    int lvl = lower_bound(all(b),cnt) - b.begin();
    if (b[lvl]<=cnt)lvl++;
    ans = max(ans,e*lvl);
    // dbg(cnt,lvl,e);
  }
  ...
  // while(lo<hi){
  //   int mi = (lo+hi)/2;
  //   int cnt = upper_bound(all(a),mi) - a.begin();
  //   cnt = n-cnt;
  //   int lvl
  // }
}
