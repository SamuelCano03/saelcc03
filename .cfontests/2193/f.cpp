...
void solve(int caso){
  ...
  mn.pb(ay),mx.pb(ay);
  fore(i,n){
    if(xa==v[i-1].ff)mn.back()=min(mn.back(),v[i-1].ss),mx.back()=max(mx.back(),v[i-1].ss);
    else xa=v[i-1].ff,mx.pb(v[i-1].ss),mn.pb(v[i-1].ss),x.pb(v[i-1].ff);
  }
  int ans = 0;
  mn.pb(by),mx.pb(by);
  x.pb(bx);
  vvi dp(x.size(),vi(2,-1));
  dp[0][0]=0;
  dp[0][1]=0;
  int pre = ax;
  // dbg(sz(x),sz(dp));
  fore(i,sz(x)-1){
    dp[i][0] = min(dp[i-1][0]+abs(mx[i-1]-mn[i]),dp[i-1][1]+abs(mn[i-1]-mn[i]))+mx[i]-mn[i]+x[i]-x[i-1];
    dp[i][1] = min(dp[i-1][0]+abs(mx[i-1]-mx[i]),dp[i-1][1]+abs(mn[i-1]-mx[i]))+mx[i]-mn[i]+x[i]-x[i-1];
  }
  ps(min(dp.back()[0],dp.back()[1]));
  return;
}
