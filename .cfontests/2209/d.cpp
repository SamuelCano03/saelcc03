...
void solve(int caso){
  ...
  read(r,g,bl);
  string s;
  vector<pii>v = {{r,'R'},{g,'G'},{bl,'B'}};
  sort(all(v));
  char a,b,c;
  c = v[0].ss, b = v[1].ss, a = v[2].ss;
  mpii mp;
  mp[c] = v[0].ff, mp[b]=v[1].ff, mp[a]=v[2].ff;
  // dbg(a,b,c);
  if(r>=g+bl or g>=r+bl or bl>=r+g){
    while(mp[b]--)s.pb(a),s.pb(b), mp[a]--;
    while(mp[c]--)s.pb(a),s.pb(c),mp[a]--;
    if(mp[a])s.pb(a);
    ps(s);
  }
  else{
    ...
    while(mp[a] and mp[b]){
      if(mp[a] and a!=s.back() and (sz(s)>=3 and s[sz(s)-3]!=a))s.pb(a),mp[a]--;
      else if(mp[b] and b!=s.back() and (sz(s)>=3 and s[sz(s)-3]!=b))s.pb(b),mp[b]--;
      else break;
    }
    while(mp[a] or mp[b]){

    if(a!=s.front() and mp[a])s.insert(s.begin(),a);
    if(b!=s.front() and mp[b])s.insert(s.begin(),b);
    }
    ps(s);
  }
}
