...
void solve(int caso){
  ...
  dbg(v[2]);
  vector<vector<pair<char,int>>>mat(m,vector<pair<char,int>>(n,{' ',-1}));
  int kmx = 0;
  fora(i,n){
    int id = i;
    fora(j,m){
    ...
    int ch  = b[i]-'a';
    if(v[ch].empty()) return ps(-1);
    int jd = upper_bound(all(v[ch]),id) - v[ch].begin() ;
    if(jd== 0) return ps(-1);
    jd--;
    int njd = v[ch][jd];
    if(i-njd>m)return ps(-1);
    kmx = max(kmx, i-njd);
    fori(k,i-njd){
      mat[k][njd+k+1] = {b[i],njd};
    }
  }
  fori(j,m)dbg(mat[j]);
  string pre = a;
