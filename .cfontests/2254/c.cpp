...
void solve2(int caso){
  ...
  string a,b; read(a,b);
  vi ax,bx,ay,by;
  for(int i=0;i<n;i+=2)ax.pb(a[i]),bx.pb(b[i]);
  for(int i=1;i<n;i+=2)ay.pb(a[i]),by.pb(b[i]);
  int x=fx(ax,bx);
  int y=fx(ay,by);
  if(x==-1 or  y==-1)return ps(-1);
  ps(x+y);
}
