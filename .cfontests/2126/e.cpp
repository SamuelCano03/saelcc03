...
void solve(int caso){
  read(n);
  vi pr(n),sf(n); read(pr,sf);
  bool fg = true;
  fora(i,n-1) if(pr[i]%pr[i+1]!=0)return nosi;
  fori(i,n-1) if(sf[i+1]%sf[i]!=0)return nosi;
  if(sf[0]!=pr.back())return nosi;
  fori(i,n-1){
    if(gcd(pr[i],sf[i+1])!=sf[0]) return nosi;
  }
  yesi;
}
