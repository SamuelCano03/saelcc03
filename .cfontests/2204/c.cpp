...
void solve(int caso){
  ...
  int yab =m/lcm(a,b), yac = m/lcm(a,c), ybc = m/lcm(b,c);
  int z = m/lcm(lcm(a,b),c);
  int A = 6*(xa-yab-yac+z) + 3*(yab-z) + 3*(yac-z) + 2*z;
  int B = 6*(xb-yab-ybc+z) + 3*(yab-z) + 3*(ybc-z) + 2*z;
  int C = 6*(xc-ybc-yac+z) + 3*(ybc-z) + 3*(yac-z) + 2*z;
  // ps(xa,xb,xc);
  // ps(yab,yac,ybc);
  // ps(z);
  ps(A,B,C);
