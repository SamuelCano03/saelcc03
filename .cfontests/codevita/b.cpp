...
void solve(int caso){
  ...
  n = sz(num);
  fori(i,n){
    if(str[i]=='S'){st.insert(num[i]-'0');continue;}
    if(num[i]=='8' or num[i]=='0' or num[i]=='1')st.insert(num[i]-'0');
    else{
      if(num[i]=='2')st.insert(5);
      if(num[i]=='5')st.insert(2);
      if(num[i]=='3' and (str[i]=='U' or str[i]=='D'))st.insert(3);
    }
  }
  string ans = "";
  mpii mp;
  for(int e: st)mp[e]++;
  fore(i,9){
    if(mp[i]>0){ans.pb(i+'0');mp[i]--;break;}
  }
  fori(i,10){
    while(mp[i]>0){
      ans.pb(i+'0');
  ...
  if(ans=="")cout<<"";
  else if(ans[0]=='0' and ans.back()=='0')cout<<0;
  else cout<<ans;
}
