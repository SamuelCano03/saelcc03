...
void solve(int caso){
  ...
  fori(ii,n){
    ...
    for(int i=0;i<sz(s);){
      string num = "";
      while(s[i]>='0' and s[i]<='9')num.pb(s[i++]);
      int nm = stoll(num);
      char color = s[i++];
      for(int k=0;k<nm;k++){
        id[ii][j+k] = idd;
  ...
  // fori(i,n)ps(id[i]);
  // ps();
  // fori(i,n){fori(j,n)cout<<char(co[i][j]); ps();}
  int sid = 0, tid = 0;
  fori(i,n)fori(j,n){
    if(co[i][j]=='S')sid = id[i][j];
    if(co[i][j]=='D')tid = id[i][j];
  }
  vi dis(idd,1e9);
  dis[sid]=0;
  queue<int>q;
  ...
  while(q.size()){
    ...
    fori(i,n){
      fori(j,n){
        if(id[i][j]!=u) continue;
        fori(ii,4){
          int xi = x[ii], yi = y[ii];
          if(i+xi>=0 and i+xi<n and j+yi>=0 and j+yi<n and id[i+xi][j+yi]!=u and co[i+xi][j+yi]!='R') st.insert(id[i+xi][j+yi]);
        }
      }
    }
    dbg(u);
