...
void solve(int caso){
  ...
  if(sz(s2)==0)return ps(0);
  int sum =0;
  for(int e: s2)sum +=e;
  int ans = sum;
  for(auto [key,val]:mp) if(val>0)ax.pb(key);
  sort(all(ax));
  reverse(all(ax));
  fori(i,sz(ax))
    if(ax[i]<sum) {ans = ax[i]+sum; break;}
  fori(i,sz(ax)-1){
    if(ax[i]<ax[i+1]+sum){ans = ax[i]+ax[i+1]+sum; break;}
  }
  if(ans==sum and cnt<=2)ans=0;
  ps(ans);
}
