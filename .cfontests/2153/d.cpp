...
bool fx(vi &v, int tol){
  ...
  fori(i,n){
    ...
    // if(tol==2 and i==3) dbg(r[j],v[i],l[k]);
    // if(abs(v[i]-v[j])>tol and abs(v[i]-v[k])>tol)return false;
    if(abs(v[i]-v[j])>2*tol)return false;
  }
  return true;
}
void solve(int caso){
  fast;
  // a;lksfdj;asdfklja;sldf;sfsl;dfkj as;ldfk ja;dfkj
  // priority_queue<pii>q;
  // fori(i,n){
  //   int l = abs(v[i]-v[(i-1+n)%n]);
  //   int r = abs(v[i]-v[(i+1)%n]);
  //   q.push({min(l,r),i});
  // }
  // while(q.size()){
