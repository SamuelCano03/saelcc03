...
void pre(){
  fore(i, 50) f[i] = f[i-1]*i %mod;
  int x,y;
  fore(i, 50) {
    int g =  gcd(f[i],mod,x,y);
    if(g!=1)ps("no solu");
    inv[i] = (x%mod+mod)%mod;
  }
}
int comb(int a, int b){
  return ((f[a]*inv[b]%mod)*inv[a-b]%mod+mod)%mod;
}
void solve(int caso){
  read(n);
  vi v(n+1); read(v);
  int x = v[0], sm = accumulate(all(v),0ll);  
  int k = sm/n;
  fore(i,n)if(v[i]>k+1)return ps(0);
  fore(i,n){
    if(v[i]>=k)continue;
    x -= (k-v[i]);
    if(x<0) return ps(0);
  ...
  x = v[0];
  int a = sm%n,b=n-sm%n,c=0;
  fore(i,n)if(v[i]<k)x-=k-v[i],v[i]=k;
  fore(i,n)if(v[i]==k)c++;
  // dbg(a,b,c,f[a],f[b],f[c]);
  int ans = ((f[a]*f[b])%mod * comb(c,x)%mod+mod)%mod;
  ps(ans);
}
