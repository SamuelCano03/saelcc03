...
void solve(int caso){
  ...
  map<int,vi>mp;
  pre[0] = (s[0]=='a'?1:-1);
  fore(i,n-1)pre[i] = pre[i-1] + (s[i]=='a'?1:-1);
  fori(i,n)mp[pre[i]].pb(i);
  int dif = pre.back(), ans = n;
  if(dif==0)return ps(0);
  fori(i,n){
    int x = pre[i], y = pre[i] + dif + (dif>0?-1:1);
    auto j = lower_bound(all(mp[y]),i);
    if(j==mp[y].end())continue;
    if(dif>0 and s[i]=='b') continue;
    else if(dif<0 and s[i]=='a')continue;
    ans = min(ans,*j-i+1);
  }
  if(ans==n)ans=-1;
  ps(ans);
