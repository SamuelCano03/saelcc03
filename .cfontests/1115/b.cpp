...
void solve(int caso){
  ...
  if(abs(zer-one)<=1)return ps(zer+one);
  if(abs(zer-one)>3)return ps(-1);
  c = (one<zer ? '1': '0');
  if(s[0]==c and s.back()==c)return ps(zer+one+abs(zer-one)-1);
  if((s[0]==c or s.back()==c) and abs(zer-one)==2) return ps(zer+one+abs(zer-one)-1);
  return ps(-1);
}
