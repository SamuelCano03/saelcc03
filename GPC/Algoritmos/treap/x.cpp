...
void solve(int caso){
  ...
  sort(all(q));
  map<int,vi>mp;
  fori(i,n)mp[v[i]].pb(i);
  treap tp;
  for(auto &ve: q){
    int b=ve[0],a=ve[1],c=ve[2],i=ve[3];
    c--;a--;
    while(siz(tp.root)<=b)tp.insert(v[siz(tp.root)]);
    int e = tp.kth(a);
    ans[i] = mp[e].size()<=c?-1:mp[e][c];
  }
  for(auto e: ans)ps(e);
}
void solve2(int caso){
  ...
  for(auto &ve: q){
    int b=ve[0],a=ve[1],c=ve[2],i=ve[3];
    c--;a--;
    while(st.size()<=b) st.insert(v[st.size()]);
    int e = *st.find_by_order(a);
    ans[i] = (mp[e].size()<=c?-1:mp[e][c]);
  }
  for(auto e: ans)ps(e);
}
